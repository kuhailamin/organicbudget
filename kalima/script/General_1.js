...
function General() {
    ...
    this.COURSE_NAVIGATION_WIDTH = 230;
...
General.prototype.show_lessons = function (data, course_link) {
    ...
    $.ajax({
        url: constants.ROOT_PATH + "/courses/" + course_link + "/lessons/index.html",
        data: 'foo',
        success: function (data) {
            $(GENERAL.COURSE_CONTENT_DIV_ID).html(data);
            $(GENERAL.COURSE_CONTENT_DIV_ID).children().last().css("margin-bottom", "200px");
        },
        cache: false
    });
...
General.prototype.add_prism_script=function(){
/*$.getScript(constants.ROOT_PATH+"courses/script/prism.js", function(){
    $(GENERAL.COURSE_CONTENT_DIV_ID+" pre").scroll(function(){
        $(this).trigger("click");
        alert("clicked");
    }); 
});*/
    $(GENERAL.COURSE_CONTENT_DIV_ID+" pre").on("touchmove",function(){
        $(this).trigger("click");
        alert("phone touched");
    }); 
    $(GENERAL.COURSE_CONTENT_DIV_ID+" pre").on("click",function(){
        //$(this).trigger("click");
        alert("phone clicked");
    });     
